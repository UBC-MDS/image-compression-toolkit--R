...
test_that("function raises value error when invalid values passed in for height and width", {
  expect_error(crop(image, -1, 10,out_img), "ValueError")
  expect_error(crop(image, 10, -1,out_img), "ValueError")
  expect_error(crop(image, 0, 0,out_img), "ValueError")
  expect_error(crop(image, 0, 10,out_img), "ValueError")
  expect_error(crop(image, 10, 0,out_img), "ValueError")
  expect_error(crop(image, dim(readImage(image))[1] + 1, 10,out_img), "ValueError")
  expect_error(crop(image, 10, dim(readImage(image))[2] + 1,out_img), "ValueError")
  expect_error(crop(image, -10000, 10,out_img), "ValueError")
  expect_error(crop(image, 10, -10,out_img), "ValueError")
