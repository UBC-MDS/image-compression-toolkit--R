...
test_that("function returns the correct output shape", {
  expect_equal(dim(readImage(crop(image, as.integer(15), 10L)))[1:2], c(15, 10))
  expect_equal(dim(readImage(crop(image, as.integer(10), as.integer(5))))[1:2], c(10, 5))
  expect_equal(dim(readImage(crop(image, as.integer(6), as.integer(6))))[1:2], c(6, 6))
  expect_equal(dim(readImage(crop(image, as.integer(5), as.integer(5))))[1:2], c(5, 5))
  expect_equal(dim(readImage(crop(image, as.integer(3), as.integer(7))))[1:2], c(3, 7))
...
test_that("function raises value error when invalid values passed in for height and width", {
  ...
  expect_error(crop(image, dim(readImage(image))[1] + 1, 10), "ValueError")
  expect_error(crop(image, 10, dim(readImage(image))[2] + 1), "ValueError")
...
test_that("function raises type error when invalid types are passed in", {
  ...
  expect_error(crop(image, -9.9, -4.5), "ValueError")
  expect_error(crop(image, "10", TRUE), "TypeError")
  #expect_error(crop(55, 10, 10), "TypeError")
})
